...
This project works by pulling from the NAP database and uses that time in synchronisation with the internal ESP32 clock to give me an accurate time. To reduce battery usage and to make sure the device is not getting too hot, it is updated every 10 seconds. Ensuring that the clock is always in time.
